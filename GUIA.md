...
- **Prédio(s) que já existe(m) no cadastro** — antes de perguntar, o sistema já calcula uma **prévia com números reais**: quantos equipamentos serão atualizados, quantos são novos, quantos sumiram da planilha. Se o número de "novos" parecer alto demais, pode ser um erro de digitação no Patrimônio da planilha (em vez de casar com quem já existe, criaria um cadastro duplicado) — confira antes de confirmar. Confirmando, **ATUALIZA** o cadastro: casa cada linha pelo número de **Patrimônio** — quem já existe tem os dados de cadastro corrigidos/completados (setor, ambiente, marca, modelo, capacidade, tipo de gás, status), e quem for realmente novo é adicionado e entra automaticamente no cronograma. **Nada é apagado** — o status/data/equipe já agendados de quem já existia nunca são tocados. Se sobrar algum equipamento cadastrado que não apareceu na planilha, aparece um card **"Sumiram da última planilha"** na tela de Levantamento, listando quem foi — você decide remover na mão, se quiser (o sistema nunca apaga sozinho).
